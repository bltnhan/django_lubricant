...
# app = DjangoDash(f'{file_name}', [dbc.themes.BOOTSTRAP],
...
@app.callback(
    [Output('my_datatable_collapse', 'data'),
...
def update_graph(my_dd_year, my_dd_month, my_dd_class, my_dd_segment):
    ...
    table_main = data_table.groupby(by='CLASS').agg({'VOLUME': 'sum', 'TOTAL_AMT': 'sum'}).reset_index()
    table_detail = data_table[['TAX_CODE', 'IMPORTER_NAME', 'SEGMENT', 'VOLUME', 'TOTAL_AMT']]
    ...
    table_detail['VOLUME'] = table_detail['VOLUME'].map('{:,.2f}'.format)
    table_detail['TOTAL_AMT'] = table_detail['TOTAL_AMT'].map('${:,.2f}'.format)
    return table_detail.to_dict('records'), vol, amount, fig_vol, fig_amount, table_main.to_dict('records'), title_
